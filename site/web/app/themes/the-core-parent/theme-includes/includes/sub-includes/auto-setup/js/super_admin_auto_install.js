...
(function ($) {
	...
	autoSetupProcess = {
		...
		initialize: function ($infoContainer) {
			...
			jQuery(window).on('beforeunload', autoSetupProcess.onLeaveAlert);
		...
		processFailed: function (jqXHR, textStatus) {
			...
			jQuery(window).off('beforeunload', autoSetupProcess.onLeaveAlert);
		},
		...
		activate_required_plugins: function () {
			...
			return jQuery.ajax({
				...
				jQuery.get(auto_setup_data.admin_url + '?fake=true', function () {
	...
	jQuery(document).on('ready', function () {
		autoSetupProcess.initialize(jQuery('.wrap'));
	});
