...
class FW_Admin_Plugins_Install {
	private $page_slug = 'themefuse_auto_setup';
	private $file_uri;
	private $file_path;
	private $config_path;
	private $view_path;
		...
		if ( ! empty( $this->demos ) ) {
			...
			wp_localize_script(
				...
				array(
					...
					'messages'     => array(
						'installing'              => __( 'Installing', 'the-core' ),
						'start_install_plugins'   => __( 'Installing required plugins ...', 'the-core' ),
						'finish_install_plugins'  => __( 'Finished installing required plugins', 'the-core' ),
						'start_activate_plugins'  => __( 'Activating required plugins ...', 'the-core' ),
						'finish_activate_plugins' => __( 'Finished activating required plugins', 'the-core' ),
					)
				)
			);
		}
	...
	public function get_demo_steps() {
		return array(
			'activate-demo-plugins' => array(
				...
				'message'     => esc_html__( 'Activate required demo plugins', 'the-core' ),
			),
			'install-demo-plugins'  => array(
				'ajax_action' => $this->_prefix . '_install_demo_plugins',
				'nonce'       => wp_create_nonce( 'install-demo-plugins' ),
				'message'     => esc_html__( 'Install required demo plugins', 'the-core' ),
			),
		);
	}
	...
	public function install_demo_plugins() {
		...
		if ( ! $this->initialize_filesystem( $credentials, WP_PLUGIN_DIR ) ) {
			...
			wp_send_json_error( array(
				'message' => sprintf( esc_html__( 'Failed to install required plugins. Folder %s is not writable', 'the-core' ), WP_PLUGIN_DIR ),
			) );
		}
		...
		if ( ! empty( $failed_plugins ) ) {
			...
			wp_send_json_error( array(
				'message' => sprintf( esc_html__( 'Failed to install required plugins. %s', 'the-core' ), implode( ', ', $failed_plugins ) ),
			) );
		}
		ob_end_clean();
		wp_send_json_success();
	...
	public function activate_plugins( $plugins ) {
		if ( ! current_user_can( 'activate_plugins' ) ) {
			return new WP_Error( 0, __( 'Current user can\'t activate plugins', 'the-core' ) );
		}
		...
		if ( ! empty( $failed_plugins ) ) {
			return new WP_Error( 0, sprintf(
					esc_html__( 'Failed to activate required plugins. %s', 'the-core' ),
	...
	public function item_menu_page() {
		add_theme_page( 'TF Plugin Updates', 'TF Plugin Updates', 'manage_options', $this->page_slug, array(
			$this,
			'auto_setup_page'
		) );
	}
	...
	public function install_required_plugins() {
		...
		$response = FW_Plugin_Installer_Helper::bulk_install( $this->get_required_plugins(), true ); // force install plugins
	...
	public function get_setup_messages() {
		return array(
			'plugins_only'      => sprintf( esc_html__( 'This option will activate Unyson dependencies. The %s demo content will not be installed %s.', 'the-core' ), '<b>', '</b>' ),
			'plugins_and_demo'  => sprintf( esc_html__( 'This option will activate Unyson dependencies together %s with the demo content %s for the theme.', 'the-core' ), '<b>', '</b>' ),
			'skip_auto_install' => esc_html__( 'Skip the auto setup all together and activate all the Unyson dependencies manually. Note that this page will not be  accessible until you install the theme again.', 'the-core' )
		);
	}
	...
	private function admin_enqueue_scripts() {
		...
		$auto_setup_data = array(
			...
			'messages'                  => array(
				'on_leave_alert'    => esc_html__( 'Attention, the installation process is not yet finished, if leave this page, you lose the information stored on the site!', 'the-core' ),
				'server_problems'   => esc_html__( "Sorry, we've encountered some errors, try to access this page later.", "the-core" ),
				'process_completed' => esc_html__( 'The installation process was completed successfully.', 'the-core' )
			),
		);
	...
	public function activate_supported_extensions() {
		...
		if ( is_array( $result ) ) {
			...
			if ( ! empty( $failed_extensions ) ) {
				...
				wp_send_json_error( array(
					'message' => sprintf( esc_html__( 'Failed to activate supported extensions. %s ', 'the-core' ), implode( ', ', $failed_extensions ) ),
				) );
			}
		}
	...
	protected function get_steps() {
		return array(
			'activate-supported-extensions' => array(
				...
				'message'     => esc_html__( 'Activating supported extensions', 'the-core' )
			),
			'finish-install-process'        => array(
				'ajax_action' => $this->_prefix . '_finish_install_process',
				'nonce'       => wp_create_nonce( 'finish-install-process' ),
...
class FW_Admin_Auto_Install extends FW_Simple_Auto_Install {
		...
		return array_merge(
			...
			array(
				'activate-unyson'           => array(
					...
					'message'     => esc_html__( 'Activating Unyson', 'the-core' ),
				),
				'activate-required-plugins' => array(
					...
					'message'     => esc_html__( 'Activate required plugins', 'the-core' ),
				)
			)
		);
	}
	...
	public function get_setup_messages() {
		return array(
			'plugins_only'      => sprintf( esc_html__( 'This option will activate %s already installed plugins and dependencies %s by Network Administrator. The %s demo content will not be installed %s.', 'the-core' ), '<b>', '</b>', '<b>', '</b>' ),
			'plugins_and_demo'  => sprintf( esc_html__( 'This option will activate Unyson dependencies together %s with the demo content %s for the theme.', 'the-core' ), '<b>', '</b>' ),
			'skip_auto_install' => esc_html__( 'Skip the auto setup all together and activate all the plugins manually. Note that this page will not be accessible until you install the theme again.', 'the-core' )
		);
	}
...
class FW_Super_Admin_Auto_Install extends FW_Admin_Auto_Install {
		...
		return array_merge(
			...
			array(
				'install-unyson'               => array(
					...
					'message'     => esc_html__( 'Downloading and installing Unyson', 'the-core' ),
				),
				'install-required-plugins'     => array(
					'ajax_action' => $this->_prefix . '_install_required_plugins',
					'nonce'       => wp_create_nonce( 'install-required-plugins' ),
				...
				'install-supported-extensions' => array(
					...
					'message'     => esc_html__( 'Installing supported extensions', 'the-core' ),
				),
				'install-child-theme'          => array(
					'ajax_action' => $this->_prefix . '_install_child_theme',
					'nonce'       => wp_create_nonce( 'install-child-theme' ),
					'message'     => esc_html__( 'Downloading and installing child theme', 'the-core' ),
				),
			)
		);
	}
	...
	public function install_supported_extensions() {
		...
		if ( ! $this->initialize_filesystem( $credentials, WP_PLUGIN_DIR ) ) {
			wp_send_json_error( array(
				'message' => sprintf( esc_html__( 'Failed to install supported extensions. Folder %s is not writable', 'the-core' ), WP_PLUGIN_DIR ),
			) );
		}
		...
		if ( is_array( $result ) ) {
			...
			if ( ! empty( $failed_extensions ) ) {
				...
				wp_send_json_error( array(
					'message' => sprintf( esc_html__( 'Failed to install supported extensions. %s ', 'the-core' ), implode( ', ', $failed_extensions ) ),
				) );
			}
		}
	...
	public function install_unyson() {
		...
		if ( ! $this->initialize_filesystem( $credentials, WP_PLUGIN_DIR ) ) {
			wp_send_json_error( array(
				'message' => sprintf( esc_html__( 'Failed to install Unyson. Folder %s is not writable', 'the-core' ), WP_PLUGIN_DIR ),
			) );
		}
	...
	public function install_child_theme() {
		...
		if ( ! $this->initialize_filesystem( $credentials, wp_normalize_path( get_theme_root() ) ) ) {
			wp_send_json_error( array(
				'message' => sprintf( esc_html__( 'Failed to install Theme. Folder %s is not writable', 'the-core' ), wp_normalize_path( get_theme_root() ) ),
			) );
		}
	...
	public function get_setup_messages() {
		return array(
			'plugins_and_demo'  => sprintf( esc_html__( 'This option will install and activate all the plugins and dependencies together %s with the demo content %s for the theme.', 'the-core' ), '<b>', '</b>' ),
			'plugins_only'      => sprintf( esc_html__( 'This option will install and activate all the plugins and dependencies. The %s demo content will not be installed %s.', 'the-core' ), '<b>', '</b>' ),
			'skip_auto_install' => esc_html__( 'Skip the auto setup all together and install all the plugins manually. Note that this page will not be accessible until you install the theme again.', 'the-core' )
		);
	}
}
...
class FW_Plugin_Installer_Helper extends FW_Installer_Helper {
	...
	public static function install( $plugin, $force = false ) {
		return self::is_installed( $plugin ) && ! $force
			? array(
				'success' => true,
				'data'    => array(
					'message' => esc_html__( 'Plugin is already installed.', 'the-core' )
				)
			)
	...
	public static function activate( $plugin ) {
		if ( ! self::is_installed( $plugin ) ) {
			return array(
				'success' => false,
				'data'    => array(
					'message' => sprintf( esc_html__( 'It was not possible to activate %s. Because it isn\'t installed.', 'the-core' ), $plugin['name'] )
				)
			);
		}
		...
		/* skip temporary the plugins that on activate return errors (need to comment this code) */
	...
	public static function bulk_activate( $plugins ) {
		...
		if ( ! current_user_can( 'activate_plugins' ) ) {
			return array(
				'message' => sprintf( esc_html__( 'Current user can\'t activate plugins.', 'the-core' ), WP_PLUGIN_DIR ),
			);
		}
	...
	private static function api( $action, $args = null ) {
		...
		if ( false === $res ) {
			...
			if ( is_wp_error( $request ) ) {
				$res = new WP_Error( 'plugins_api_failed', esc_html__( 'An unexpected error occurred. Something may be wrong with WordPress.org or this server&#8217;s configuration. If you continue to have problems, please try the <a href="https://wordpress.org/support/">support forums</a>.', 'the-core' ), $request->get_error_message() );
			} else {
				$res = maybe_unserialize( wp_remote_retrieve_body( $request ) );
				if ( ! is_object( $res ) && ! is_array( $res ) ) {
					$res = new WP_Error( 'plugins_api_failed', esc_html__( 'An unexpected error occurred. Something may be wrong with WordPress.org or this server&#8217;s configuration. If you continue to have problems, please try the <a href="https://wordpress.org/support/">support forums</a>.', 'the-core' ), wp_remote_retrieve_body( $request ) );
				}
			}
